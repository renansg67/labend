...
def inspecao_de_arvores():
    ...
    col3.warning("""
    🌴 **I Congresso de Manejo e Conservação de Árvores Urbanas – Painel 3 (24/03)** **Tema:** Manutenção de Árvores Urbanas e Avaliação do Risco  
    **Mediador:** Rogério Bobrowski (UNICENTRO)  
    ...
    col2.write("Os principais ensaios não destrutivos para inspeção de árvores urbanas estão em conformidade com a **ABNT NBR 16246-3:2019**. Ela trata da avaliação de risco das árvores urbanas, especificando as práticas e equipamentos que devem ser utilizados em cada nível de inspeção.")

    col2.header("Inspeção técnica nível 3")

    col2.write("""
        A **Inspeção Técnica de Nível 3** é a mais avançada entre todas as técnicas. Cabe aos técnicos e profissionais que a executam realizar a inspeção com as técnicas de **Nível 2** visando ter um panorama geral da árvore inspecionada.
    """)

    col2.markdown("#### Inspeção de Nível 2 (Etapas e Equipamentos)")

    col2.write("A Inspeção de Nível 2 consiste em etapas como: Inspeção em 360° ao redor da árvore visando identificar locais com fragilidades como:")
    col2.markdown("""
        * Ocos, fissuras e galhos secos.
        * Crescimento desordenado de fungos.
        * Perda de vitalidade.
        * Ataque de insetos xilófagos.
    """)

    col2.write("Para isso, o profissional pode lançar mão de equipamentos como:")
    col2.markdown("""
        * Binóculos e trena florestal.
        * Clinômetro¹, Hipsômetro² e Trado de Incremento³ (para extração de amostras do tronco).
    """)

    col2.write("A utilização de martelo emborrachado para o **ensaio de percussão** pode ser feita objetivando encontrar locais com **som cavo** que possam indicar fragilidade estrutural, seja por biodeterioração ou cavidades.")
    ...
    col3.info("³Dispositivo manual normalmente utilizado para extrair amostras à altura do peito do operador, tendo em vista a necessidade de um posicionamento adequado para vencer a resistência do tronco durante o processo de extração das amostras cilíndricas.") # Corrigido o erro de sintaxe original na nota.
    ...
    col2.markdown("""
    * Sinais de estresse fisiológico (desfolha, clorose ou necrose foliar).
    * Falhas estruturais (rachaduras e cavidades no tronco ou nas ramificações).
    ...
    * Interferências com edificações, redes elétricas ou mobiliário urbano.
    ...
    col2.write("Apesar de suas vantagens, o uso de drones em inspeções arbóreas apresenta **limitações**:")
    col2.markdown("""
    * Dificuldade de visualização de defeitos internos (ocultos no tronco ou nas raízes).
    * Dependência de condições climáticas favoráveis, especialmente em relação à velocidade do vento e à luminosidade.
    * Sua utilização deve estar de acordo com as normas vigentes da **Agência Nacional de Aviação Civil (ANAC)**, que regulamenta o voo de aeronaves remotamente pilotadas em áreas urbanas.
    """)
    ...
    col2.write("As técnicas de escalada arbórea seguem princípios semelhantes aos utilizados em trabalhos verticais em edificações, sendo indispensável o uso de **equipamentos de proteção individual (EPI)** e de **sistemas de ancoragem** que garantam redundância e segurança. Os principais equipamentos incluem:")
    
    col2.markdown("""
        * Capacete com jugular.
        * Cinturão tipo paraquedista.
        * Cordas dinâmicas ou semiestáticas.
        * Mosquetões com trava, talabartes e freios de descida.
    """)
    col2.write("Para maior segurança, recomenda-se a utilização de dois pontos independentes de ancoragem durante a execução do procedimento.")
    ...
    col2.write("O uso da espada de ar apresenta diversas vantagens em relação a métodos tradicionais de escavação. Enquanto estes podem provocar lesões graves às raízes ou até mesmo comprometer a estabilidade da árvore, a técnica com ar comprimido **preserva a integridade fisiológica** do sistema radicular, permitindo que a planta mantenha suas funções vitais. Além disso, o procedimento é menos invasivo e possibilita a recuperação mais rápida do solo após a inspeção.")
    ...
    col2.write("""
        A Tomografia é uma técnica essencial na inspeção de árvores para a avaliação da **qualidade interna do tronco**, diferenciando regiões de madeira sã, deterioração ou cavidades .
        
        O método mais comum (Tomografia Acústica) utiliza um equipamento multissensor (acelerômetros) que mede a **velocidade de propagação de ondas** no interior do tronco. Múltiplos sensores captam o sinal em diferentes rotas.
        
        * **Velocidades Lentas:** Podem indicar cavidades ou deterioração (baixa rigidez).
        * **Velocidades Altas:** Representam madeira sã, com rigidez e resistência adequadas.
        
        Os resultados são apresentados em **gráficos bidimensionais (tomogramas)** da seção transversal do tronco (como ilustrado nas imagens), que permitem visualizar o percentual de área comprometida e sua localização.
    """)
    ...
    col2.write("O termo sonda, neste contexto, engloba diferentes métodos que podem ser utilizados para a sondagem da madeira, como:")
    col2.markdown("""
        * Espaçamento de fendas.
        * Profundidade de fissuras.
        * Retirada de amostras.
        * Verificação da dureza a partir de impactos na superfície.
    """)
    ...
    col2.markdown("""
        * Modelos de confiabilidade estrutural (utilizam parâmetros obtidos em ensaios para estimar a probabilidade de ruptura).
        * Modelos bayesianos (permitem atualizar o risco à medida que novas inspeções e dados são incorporados).
        * Distribuições estatísticas de resistência (descrevem a variabilidade da resistência da madeira em função da espécie e do estado fitossanitário).
        * Simulações de Monte Carlo (possibilitam avaliar cenários complexos, variando condições ambientais como intensidade do vento, umidade do solo e presença de defeitos internos).
    """)
    ...
    * A presença de deteriorações internas (apodrecimento, cavidades ou ataque de organismos xilófagos).
    * A densidade e a qualidade estrutural da madeira.
    """)
    ...
    col2.write("As principais vantagens da termografia incluem:")
    col2.markdown("""
        * Caráter totalmente **não invasivo** (não há necessidade de perfurações ou cortes).
        * Possibilidade de inspeção em **tempo real**.
        * Aplicabilidade em inventários arbóreos urbanos de grande escala, onde a agilidade na coleta de dados é fundamental.
    """)
